...
ですが、有志の手によりF3チップでも利用可能な [BetaFlight Performance Edition F3](https://github.com/joelucid/betaflight/releases)
 という、メモリ容量の制約から機能を限定させたカスタムファームウェアが提供されています。これによりRPMフィルターなど最新の機能も利用可能です。今回はそのファームウェアを用いた設定参考パラメータを展開致します。  
